...
class InferSentModel(nn.Module):
  ...
  def forward(self, sent_tuple):
    ...
    idx_sort = (torch.from_numpy(idx_sort).cuda()
                if self.is_cuda() else torch.from_numpy(idx_sort))
    ...
    idx_unsort = (torch.from_numpy(idx_unsort).cuda()
                  if self.is_cuda() else torch.from_numpy(idx_unsort))
  ...
  def get_word_dict(self, sentences, tokenize=True):
    ...
    word_dict = {}
    sentences = [
        s.split() if not tokenize else self.tokenize(s) for s in sentences
    ]
  ...
  def prepare_samples(self, sentences, bsize, tokenize, verbose):
    sentences = [[self.bos] + s.split() +
                 [self.eos] if not tokenize else [self.bos] + self.tokenize(s) +
                 [self.eos] for s in sentences]
  ...
  def encode(self, sentences, bsize=64, tokenize=True, verbose=False):
    tic = time.time()
    sentences, lengths, idx_sort = self.prepare_samples(sentences, bsize,
                                                        tokenize, verbose)
