...
class SentenceEncoder(Constraint):
    ...
    if self.window_size:
      ...
      for transformed_text in transformed_texts:
        ...
        starting_text_windows.append(
            starting_text.text_window_around_index(modified_index,
                                                   self.window_size))
        transformed_text_windows.append(
            transformed_text.text_window_around_index(modified_index,
                                                      self.window_size))
  ...
  def _check_constraint_many(self, transformed_texts, reference_text):
    ...
    for i, transformed_text in enumerate(transformed_texts):
      ...
      # window size.
      if (self.skip_text_shorter_than_window and
          len(transformed_text.words) < self.window_size):
        scores[i] = 1
