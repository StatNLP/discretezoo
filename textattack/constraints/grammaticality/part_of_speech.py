...
class PartOfSpeech(Constraint):
  ...
  def __init__(
    ...
    if tagger_type == "stanza":
      self._stanza_pos_tagger = stanza.Pipeline(lang="en",
  ...
  def _can_replace_pos(self, pos_a, pos_b):
    return (pos_a == pos_b) or (self.allow_verb_noun_swap and
                                set([pos_a, pos_b]) <= set(["NOUN", "VERB"]))
  ...
  def _check_constraint(self, transformed_text, reference_text):
    ...
    for i in indices:
      ...
      after_ctx = reference_text.words[i + 1:min(i +
                                                 4, len(reference_text.words))]
