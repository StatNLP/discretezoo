...
class GoalFunction(ABC):
  ...
  def get_results(self, attacked_text_list, check_skip=False):
    ...
    self.num_queries = self.model.call_count
    for attacked_text, raw_output in zip(attacked_text_list, model_outputs):
      ...
      results.append(self._goal_function_result_type()(
          attacked_text,
          raw_output,
          displayed_output,
          goal_status,
  ...
  def _call_model_uncached(self, attacked_text_list):
    ...
    assert len(inputs) == len(
        outputs), f"Got {len(outputs)} outputs for {len(inputs)} inputs"
  ...
  def _call_model(self, attacked_text_list):
    ...
    if not self.use_cache:
      ...
      uncached_list = [
          text for text in attacked_text_list
