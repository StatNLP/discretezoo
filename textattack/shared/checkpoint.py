...
class Checkpoint:
  ...
  def __repr__(self):
    ...
    args_lines = []
    recipe_set = (True if "recipe" in self.args.__dict__ and
                  self.args.__dict__["recipe"] else False)
    mutually_exclusive_args = [
        "search", "transformation", "constraints", "recipe"
    ]
    if recipe_set:
      args_lines.append(
    ...
    attack_logger_lines.append(
        utils.add_indent(
            f"(Total number of examples to attack): {self.args.num_examples}",
            2))
    attack_logger_lines.append(
        utils.add_indent(f"(Number of attacks performed): {self.results_count}",
                         2))
    attack_logger_lines.append(
        utils.add_indent(
  ...
  def num_skipped_attacks(self):
    return sum(
        isinstance(r, SkippedAttackResult) for r in self.log_manager.results)

  @property
  def num_failed_attacks(self):
    return sum(
        isinstance(r, FailedAttackResult) for r in self.log_manager.results)
  ...
  def save(self, quiet=False):
    ...
    if not quiet:
      print("\n\n" + "=" * 125)
      logger.info('Saving checkpoint under "{}" at {} after {} attacks.'.format(
  ...
  def _verify(self):
    ...
    assert len(
        results_set) == self.results_count, "Duplicate AttackResults found."
