...
class AttackedText:
  ...
  def __init__(self, text_input, attack_attrs=None):
    ...
    self.attack_attrs.setdefault("original_index_map",
                                 np.arange(self.num_words))
    ...
    for key in self.attack_attrs:
      if key not in other.attack_attrs:
        ...
        if not (self.attack_attrs[key].shape == other.attack_attrs[key].shape):
          ...
          return False
      elif key == 'previous_attacked_text':
        if not id(self.attack_attrs[key]) == id(other.attack_attrs[key]):
          return False
      else:
  ...
  def pos_of_word_index(self, desired_word_idx):
    ...
    if not self._pos_tags:
      sentence = Sentence(self.text,
                          use_tokenizer=textattack.shared.utils.words_from_text)
    ...
    for word_idx, word in enumerate(self.words):
      assert (word in flair_word_list
  ...
  def align_with_model_tokens(self, model_wrapper):
    """Align AttackedText's `words` with target model's tokenization scheme
        ...
        """
    tokens = model_wrapper.tokenize([self.tokenizer_input],
                                    strip_prefix=True)[0]
    word2token_mapping = {}
    j = 0
    last_matched = 0
  ...
  def printable_text(self, key_color="bold", key_color_method=None):
    ...
    # color the key.
      ...
      if key_color_method:
        ...
        def ck(k):
          return textattack.shared.utils.color_text(k, key_color,
                                                    key_color_method)
