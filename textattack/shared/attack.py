...
class Attack:
  ...
  def __init__(
    ...
    self.search_method.transformation = transformation
    if not self.transformation:
      raise NameError("Cannot instantiate attack without transformation")
    self.is_black_box = (getattr(transformation, "is_black_box", True) and
    ...
    self.search_method.pre_transformation_constraints = self.pre_transformation_constraints
    self.search_method.constraints = self.constraints
  ...
  def _get_transformations_uncached(self,
                                    current_text,
                                    original_text=None,
                                    **kwargs):
  ...
  def get_transformations(self, current_text, original_text=None, **kwargs):
    ...
    return self.filter_transformations(transformed_texts, current_text,
                                       original_text, **kwargs)

  def _filter_transformations_uncached(self,
                                       transformed_texts,
                                       current_text,
                                       original_text=None):
  ...
  def filter_transformations(self,
                             ...
                             current_text,
                             original_text=None,
                             sort_texts_alphabetically=True,
                             **kwargs):
    ...
    for transformed_text in transformed_texts:
      if (current_text, transformed_text) not in self.constraints_cache:
        ...
        # promote transformed_text to the top of the LRU cache
        self.constraints_cache[(current_text,
                                transformed_text)] = self.constraints_cache[(
  ...
  def _get_examples_from_dataset(self, dataset, indices=None):
    ...
    while indices:
      ...
      attacked_text = AttackedText(text_input,
                                   attack_attrs={"label_names": label_names})
