...
class AttackLogManager:
  ...
  def log_attack_details(self, attack_name, model_name):
    ...
    self.log_summary_rows(attack_detail_rows, "Attack Details",
                          "attack_details")
  ...
  def log_summary(self):
    ...
    num_words_changed_until_success = np.zeros(
        2**16)  # @ TODO: be smarter about this
    failed_attacks = 0
    skipped_attacks = 0
    ...
    for i, result in enumerate(self.results):
      ...
      max_words_changed = max(max_words_changed or num_words_changed,
                              num_words_changed)
      if len(result.original_result.attacked_text.words) > 0:
        perturbed_word_percentage = (
            num_words_changed * 100.0 /
    ...
    original_accuracy = (total_attacks -
                         skipped_attacks) * 100.0 / (total_attacks)
    ...
    if successful_attacks + failed_attacks == 0:
      ...
      attack_success_rate = (successful_attacks * 100.0 /
    ...
    average_perc_words_perturbed = str(round(average_perc_words_perturbed,
                                             2)) + "%"
    ...
    self.log_summary_rows(summary_table_rows, "Attack Results",
