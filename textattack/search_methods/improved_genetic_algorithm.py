...
class ImprovedGeneticAlgorithm(GeneticAlgorithm):
  ...
  def _modify_population_member(self, pop_member, new_text, new_result,
    ...
    """Modify `pop_member` by returning a new copy with `new_text`,
        ...
        given `word_idx`"""
    num_replacements_left = np.copy(
        pop_member.attributes["num_replacements_left"])
  ...
  def _crossover_operation(self, pop_member1, pop_member2):
    ...
    words_to_replace = []
    num_replacements_left = np.copy(
        pop_member1.attributes["num_replacements_left"])
    ...
    for i in range(crossover_point, pop_member1.num_words):
      ...
      num_replacements_left[i] = pop_member2.attributes[
          "num_replacements_left"][i]
  ...
  def _initialize_population(self, initial_result, pop_size):
    ...
    num_replacements_left = np.array([self.max_replace_times_per_index] *
                                     len(words))
    population = []
