...
class RandomWordSwapWIR(SearchMethod):
  ...
  def __init__(self,
    ...
    self.improvement_only = improvement_only
    self.loop_until_success = loop_until_success
    self.budget = budget
  ...
  def _perform_search(self, initial_result):
    ...
    generated_texts = [attacked_text]

    i = 0
    queries = 0
    cur_result = initial_result
    results = None
    candidate_tracker = np.zeros(len(initial_result.attacked_text.words), dtype=np.bool)
    while ((i < len(index_order) and not self.loop_until_success)
      or (queries < self.budget and self.loop_until_success)) \
      and not search_over \
      and not (queries == 0 and i == len(index_order)):
      if i >= len(index_order) and self.loop_until_success:
        i = 0
      transformed_text_candidates = self.get_transformations(
      ...
      if len(transformed_text_candidates) == 0:
        candidate_tracker[index_order[i]] = True
        i += 1
        if np.alltrue(candidate_tracker):
          break
        continue
      else:
        candidate_tracker = np.zeros_like(candidate_tracker, dtype=np.bool)
      #results, search_over = self.get_goal_results(transformed_text_candidates)
      #results = sorted(results, key=lambda x: -x.score)
      random.shuffle(transformed_text_candidates)
      sampled_candidate = transformed_text_candidates[0]
      generated_texts.append(sampled_candidate)
      results, search_over = self.get_goal_results([sampled_candidate])
      result = results[0]
      queries += 1
      # Skip swaps which don't improve the score
      if self.improvement_only:
  ...
  def extra_repr_keys(self):
    return ["wir_method", "improvement_only"]
