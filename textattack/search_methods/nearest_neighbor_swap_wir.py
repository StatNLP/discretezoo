...
class NearestWordSwapWIR(SearchMethod):
  ...
  def __init__(self, wir_method="rand", improvement_only=False, closest=True, word_embeddings=None):
    self.wir_method = wir_method
    self.improvement_only = improvement_only
    self.closest = closest
    if word_embeddings is None:
      word_embeddings = WordEmbedding.counterfitted_GLOVE_embedding()
  ...
  def _perform_search(self, initial_result):
    ...
    while i < len(index_order) and not search_over:
      current_index = index_order[i]
      current_word = attacked_text.words[current_index]
      transformed_text_candidates = self.get_transformations(
      ...
      if len(transformed_text_candidates) == 0:
        continue
      distances = [(self.word_embeddings.get_cos_sim(current_word, candidate.words[current_index]), candidate) 
      ...
      if self.closest:
        replacement = sorted_distances[0][1]
      else:
        replacement = sorted_distances[-1][1]
      
      results, search_over = self.get_goal_results([replacement])
      result = results[0]
