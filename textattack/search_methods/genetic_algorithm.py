...
class GeneticAlgorithm(PopulationBasedSearch, ABC):
  ...
  def _perturb(self, pop_member, original_result, index=None):
    ...
    while iterations < non_zero_indices:
      ...
      diff_scores = (torch.Tensor([r.score for r in new_results]) -
                     pop_member.result.score)
  ...
  def _post_crossover_check(self, new_text, parent_text1, parent_text2,
                            original_text):
    ...
    if "last_transformation" in new_text.attack_attrs:
      previous_text = (parent_text1 if "last_transformation"
                       in parent_text1.attack_attrs else parent_text2)
      passed_constraints = self._check_constraints(new_text,
                                                   previous_text,
  ...
  def _crossover(self, pop_member1, pop_member2, original_text):
    ...
    while num_tries < self.max_crossover_retries + 1:
      ...
      if self.post_crossover_check:
        passed_constraints = self._post_crossover_check(new_text, x1_text,
                                                        x2_text, original_text)
  ...
  def _perform_search(self, initial_result):
    ...
    for i in range(self.max_iters):
      ...
      if (self._search_over or population[0].result.goal_status
