...
class WordSwapChangeName(WordSwap):
  ...
  def _get_replacement_words(self, word, word_part_of_speech):
    ...
    if (tag.value in ("B-PER", "S-PER") and
        tag.score >= self.confidence_score and not self.last_only):
      replacement_words = self._get_firstname(word)
    elif (tag.value in ("E-PER", "S-PER") and
          tag.score >= self.confidence_score and not self.first_only):
