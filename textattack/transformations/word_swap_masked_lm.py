...
class WordSwapMaskedLM(WordSwap):
  ...
  def __init__(
    ...
    self._lm_tokenizer = AutoTokenizer.from_pretrained(masked_language_model,
                                                       use_fast=True)
  ...
  def _bert_attack_replacement_words(
    ...
    target_ids_pos = list(
        range(masked_index, min(masked_index + len(word_tokens),
                                self.max_length)))
    ...
    if not len(target_ids_pos):
      ...
      for bpe_tokens in products:
        ...
        word = "".join(
            self._lm_tokenizer.convert_ids_to_tokens(word_tensor)).replace(
                "##", "")
