...
def _generate_adversarial_examples(model, attack_class, dataset):
  ...
  adv_attack_results = []
  for adv_ex in tqdm.tqdm(attack.attack_dataset(dataset),
                          desc="Attack",
                          total=len(dataset)):
...
def train_model(args):
  ...
  # the training args.
  adversarial_training = (attack_class
                          is not None) and (not args.check_robustness)
  ...
  for epoch in tqdm.trange(int(args.num_train_epochs),
    ...
    if adversarial_training:
      if epoch >= args.num_clean_epochs:
        if (epoch - args.num_clean_epochs) % args.attack_period == 0:
          ...
          train_dataloader = _make_dataloader(tokenizer, adv_train_text,
    ...
    for step, batch in enumerate(prog_bar):
      ...
      if (global_step > 0 and (args.checkpoint_steps > 0) and
    ...
    if args.check_robustness:
      ...
      tb_writer.add_scalar("robustness_attack_succ_rate", adv_succ_rate,
                           global_step)
      tb_writer.add_scalar("robustness_after_attack_acc", after_attack_acc,
                           global_step)
