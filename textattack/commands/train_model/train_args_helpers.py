...
def prepare_dataset_for_training(datasets_dataset):
  ...
  text, outputs = zip(
      *((prepare_example_dict(x[0]), x[1]) for x in datasets_dataset))
...
def dataset_from_args(args):
  ...
  if args.dataset_train_split:
    ...
    try:
      train_dataset = textattack.datasets.HuggingFaceDataset(*dataset_args,
                                                             split="train")
  ...
  if args.dataset_dev_split:
    ...
    try:
      eval_dataset = textattack.datasets.HuggingFaceDataset(*dataset_args,
                                                            split="dev")
...
def attack_from_args(args):
  ...
  assert not (args.check_robustness and not (args.attack)
...
def write_readme(args, best_eval_score, best_eval_score_epoch):
  ...
  dataset_name = (args.dataset.split(ARGS_SPLIT_TOKEN)[0]
                  if ARGS_SPLIT_TOKEN in args.dataset else args.dataset)
