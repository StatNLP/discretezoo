...
def add_model_args(parser):
  ...
  model_group.add_argument(
      ...
      default=None,
      help=
      'Name of or path to a pre-trained model to attack. Usage: "--model {model}:{arg_1}={value_1},{arg_3}={value_3},...". Choices: '
      + str(model_names),
  )
  model_group.add_argument(
      "--model-from-file",
...
def load_module_from_file(file_path):
  ...
  colored_file_path = textattack.shared.utils.color_text(file_path,
                                                         color="blue",
                                                         method="ansi")
...
def parse_attack_from_args(args):
  ...
  if args.recipe:
    if ARGS_SPLIT_TOKEN in args.recipe:
      ...
      recipe = eval(
          f"{ATTACK_RECIPE_NAMES[recipe_name]}.build(model, {params})")
...
def default_checkpoint_dir():
  current_dir = os.path.dirname(os.path.realpath(__file__))
  checkpoints_dir = os.path.join(current_dir, os.pardir, os.pardir, os.pardir,
                                 "checkpoints")
