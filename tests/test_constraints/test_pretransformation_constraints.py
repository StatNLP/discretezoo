...
class TestPretransformationConstraints:
  ...
  def test_input_column_modification_basic(self, sentence_attacked_text,
  ...
  def test_input_column_modification_hypothesis(self, entailment_attacked_text):
    constraint = textattack.constraints.pre_transformation.InputColumnModification(
        ["premise", "hypothesis"],
        {"premise"}  # don't modify 'premise' column
    )
    assert constraint._get_modifiable_indices(entailment_attacked_text) == set(
        range(36, 49))
  ...
  def test_max_word_index(self, sentence_attacked_text):
    ...
    assert short_constraint._get_modifiable_indices(
        sentence_attacked_text) == set(range(8))

    long_constraint = (textattack.constraints.pre_transformation.
                       MaxWordIndexModification(8000))
    assert long_constraint._get_modifiable_indices(
        sentence_attacked_text) == set(range(len(sentence_attacked_text.words)))

  def test_repeat_modification(self, sentence_attacked_text,
                               entailment_attacked_text):
    ...
    entailment_attacked_text.attack_attrs["modified_indices"] = {
        1, 3, 11, 17, 23
    }
  ...
  def test_stopword_modification(self, sentence_attacked_text,
                                 entailment_attacked_text):
