...
def test_embedding_augmenter():
  ...
  augmenter = EmbeddingAugmenter(pct_words_to_swap=0.01,
                                 transformations_per_example=64)
...
def test_checklist_augmenter():
  ...
  augmenter = CheckListAugmenter(pct_words_to_swap=0.01,
                                 transformations_per_example=64)
...
def test_charwap_augmenter():
  ...
  augmenter = CharSwapAugmenter(pct_words_to_swap=0.01,
                                transformations_per_example=64)
...
def test_easydata_augmenter():
  ...
  augmenter = EasyDataAugmenter(pct_words_to_swap=0.01,
                                transformations_per_example=64)
  s = "Hakuna Montana"
  augmented_text_list = augmenter.augment(s)
...
def test_wordnet_augmenter():
  ...
  augmenter = WordNetAugmenter(pct_words_to_swap=0.01,
                               transformations_per_example=64)
