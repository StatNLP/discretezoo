...
raw_text_pair = collections.OrderedDict([("premise", premise),
...
class TestAttackedText:
  ...
  def test_words(self, attacked_text, pokemon_attacked_text):
    ...
    assert pokemon_attacked_text.words == [
        'the', 'threat', 'implied', 'in', 'the', 'title', 'pokémon', '4ever',
        'is', 'terrifying', 'like', 'locusts', 'in', 'a', 'horde', 'these',
  ...
  def test_window_around_index(self, attacked_text):
    ...
    assert attacked_text.text_window_around_index(5,
                                                  5) == "up stairs into a room"
    assert (
  ...
  def test_big_window_around_index(self, attacked_text):
    assert (attacked_text.text_window_around_index(0, 10**5) +
            ".") == attacked_text.text
  ...
  def test_printable_text(self, attacked_text, attacked_text_pair):
    ...
    desired_printed_pair_text = ("Premise: " + premise + "\n" + "Hypothesis: " +
                                 hypothesis)
  ...
  def test_word_deletion(self, attacked_text):
    ...
    new_text = (new_text.delete_word_at_index(0).delete_word_at_index(
        0).delete_word_at_index(0))
  ...
  def test_pair_word_insertion(self, attacked_text_pair):
    new_text = attacked_text_pair.insert_text_after_word_index(
        3, "old decrepit")
    assert new_text.text == (
  ...
  def test_modified_indices(self, attacked_text):
    new_text = attacked_text.insert_text_after_word_index(
        2, "a very long way").insert_text_after_word_index(20, "on the couch")
    assert (
        new_text.text ==
    ...
    for old_idx, new_idx in enumerate(
        new_text.attack_attrs["original_index_map"]):
      assert (attacked_text.words[old_idx]
              == new_text.words[new_idx]) or (new_idx == -1)
    new_text = (new_text.delete_word_at_index(0).delete_word_at_index(
        15).delete_word_at_index(15).delete_word_at_index(
            15).delete_word_at_index(20))
    for old_idx, new_idx in enumerate(
        new_text.attack_attrs["original_index_map"]):
      assert (attacked_text.words[old_idx]
              == new_text.words[new_idx]) or (new_idx == -1)
