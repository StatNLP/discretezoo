...
def run(model, recipe_path, txt_log_path, csv_log_path, chkpt_path=None, num_examples=None, random_seed=42):
    if not num_examples:
        if "mr" in model:
            num_examples = 500
        else:
    ...
    basic_settings = ["--attack-n", "--shuffle=False", "--disable-stdout"]
    command = ["python3", "-m", "textattack", "attack", "--model", model, "--attack-from-file", f"{recipe_path}^Attack",
            "--num-examples", str(num_examples), "--checkpoint-dir", chkpt_path,
            "--checkpoint-interval", str(CHECKPOINT_INTERVAL), "--log-to-txt", txt_log_path, "--log-to-csv", csv_log_path, "--random-seed", str(random_seed)
        ] + basic_settings
